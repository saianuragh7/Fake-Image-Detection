...
class Settings:
    def __init__(self) -> None:
        ...
        default_frontend_dist = (
            BASE_DIR / "verisight-frontend" / "dist"
            if (BASE_DIR / "verisight-frontend" / "dist").exists()
            else BASE_DIR / "frontend" / "dist"
        )
        ...
        self.scan_store_path = _resolve_env_path(os.getenv("SCAN_STORE_PATH", "").strip(), BASE_DIR / "scan_store.json")
        self.model_path = _resolve_env_path(model_path_env, BASE_DIR / "best_convnext_detector.pth")
...
def _load_model() -> tuple[nn.Module, dict[str, int]]:
    if not SETTINGS.model_path.exists():
        raise RuntimeError(f"Checkpoint not found at {SETTINGS.model_path}")
